...
yargs(hideBin(process.argv))
  .command(
    `serve [path]`,
    "Start a development server configured for single-page applications (accent.js router).",
    (yargs) => {
      return yargs
        .positional("path", {
  ...
  .command(
    `create [name]`,
    "Create a basic filesystem for an Accent.js application.",
    (yargs) => {
    ...
    (argv) => {
      ...
      if (out)
        console.info(
          boxen(`\x1b[36mCreated Project "${name}" at ${out}`, {
            padding: 1,
            borderStyle: "double",
          })
        );
