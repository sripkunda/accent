      ...
      iterator.forEach((prop, i) => {
        ...
        while ((temp = regex.exec(template)) !== null) {
          try {
            ...
            const value = AccentScopes._compile(temp[1], JSON.parse(`{ "${iterable}": ${typeof prop == 'string' ? `"${prop}"` : prop}, "index": ${i} }`), true); // get the value of the variable
...
const $els = (sel) => {
  return AccentScopes._el(sel, true);
};
