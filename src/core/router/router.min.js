(function (global, factory) {
  ...
  class Router {
        ...
        this.routes =
          typeof routes === "object"
            ...
            : await Router.#fetch(routes, "json").then((routes) => {
                document.dispatchEvent(_AccentRouterEvents.ROUTES_RECOGNIZED);
                return routes;
              });
        this.defaultPane = pane;
        await Router.route(this, location.pathname, true, pane);
    ...
    async fillPane(destination, isRoot, parameters, targetPane, childPane) {
      ...
      const content = { content: "", title: doc.title };
      isRoot
        ? window.history.replaceState(content, doc.title, path)
        : window.history.pushState(content, doc.title, path);
      ...
      this.#executeScripts(doc, route.name);
      this.#executeScripts(targetPane, route.name);
      this.configureRoutingLinks();
    ...
    async #fetchPage(routes) {
      return await new Promise((res, rej) => {
        ...
        const json = { name: data, html: await this.#fetchPageAsync(data) };
        if (!(this.routes[data].load == _AccentRouteLoadTypes.LOAD_UNCACHED))
          this.#routerCache[data] = json;
        return json;
      });
    }
    async #fetchPageAsync(route) {
      try {
        const path = this.#formatPath(`${this.routes[route]["src"]}`);
        return await Router.#fetch(path, "html");
      } catch {
        ...
        .forEach((el) => {
          ...
          el.onclick = handler
            ...
            : (event) => {
                ...
                Router.route(this, href, false, this.defaultPane);
              };
          el["__route__"] = true;
        });
    }
    ...
    static async route(router, destination, root, pane = this.defaultPane) {
      document.dispatchEvent(_AccentRouterEvents.ROUTING_STARTED);
      const route = Router._getRouteFromInput(router, destination);
      await router
        .fillPane(route.destination, root, route.dynamics, pane)
        .then((v) => {
          document.dispatchEvent(_AccentRouterEvents.ROUTING_ENDED);
        })
        .catch((e) => {
          document.dispatchEvent(_AccentRouterEvents.ROUTING_FAILED);
          throw e;
        });
    }
