(function (global, factory) {
  ...
  class Router {
    ...
    async fillPane(destination, isRoot, parameters, targetPane, childPane) {
      ...
      path = path.replace(_AccentRouterConfig.ROUTER_DYNAMIC_LINK, (_a, b) => {
        return `${
          parameters ? (parameters[b] ? `${parameters[b]}/` : "") : ""
        }`;
      });
      ...
      doc.querySelectorAll("script").forEach(async (el) => {
        ...
        try {
          ...
          if (path?.trim()) {
            ...
            if (
              !(this.routes[route].load == _AccentRouteLoadTypes.LOAD_UNCACHED)
            )
    ...
    static #formatDynamicRouterPath(path) {
      return Array.isArray(path)
        ? path.map((p) =>
            p.replace(_AccentRouterConfig.ROUTER_DYNAMIC_LINK, "")
          )
  ...
  const _AccentRouterEvents = {
    ...
    ROUTING_ENDED: new Event(
      `${_AccentRouterConfig.EVENT_PREFIX}routing-ended`
    ),
