...
/**
 * Class that stores animations and style states in teh AccentAnimate library
 */
class AccentAnimation {
    ...
    constructor(state, states, speed) {
        if (!(typeof state === 'object' && states[state])) throw Error(_AccentAnimationErrors.STATE_NOT_FOUND());
    ...
    apply(selector, callback) {
        ...
        if (!selector) throw new Error(_AccentAnimationErrors.ELEMENT_NOT_FOUND(selector)); 
     ...
     */
    setSpeed(speed) { this.speed = speed; this.#elements.forEach(e => e.style.transition = `all ${speed}s`); }
     ...
     */
    state(state, callback) { this.#updateState(state, null, callback); }
     ...
     */
    addState(name, properties) { this.states[name] = properties; }
    ...
    #animate(callback, el) {
        ...
        const update = (el) => {
            ...
            if (!prop) throw new Error(_AccentAnimationErrors.STATE_NOT_FOUND(state)); 
    ...
    static fade(fadeState, selector, speed, callback) {
        return new AccentAnimation(fadeState, {
            out: {
                opacity: 0,
            },
            in: {
